...
class AnimateClockCanvas {
    ...
    init(){
        ...
        // 填充缓存形状
        for (let i = 0; i < this.configClock.countHeart; i++) {
            let randomSize = randomInt(this.configClock.sizeMin, this.configClock.sizeMax)
            let x = randomInt(0, this.configFrame.width)
            let y = randomInt(this.configFrame.height * (1-this.configClock.heartRangeMax), this.configFrame.height * (1-this.configClock.heartRangeMin))
            this.heartBuffer.push({
                id: i,
                gravity: randomFloat(this.configClock.gravityMin, this.configClock.gravityMax),
                opacity: 0,
                opacityFinal: randomInt(this.configClock.minOpacity, this.configClock.maxOpacity), // 最终透明度
                ...
                colorH: randomInt(this.configClock.hMin, this.configClock.hMax)
            })
        }
    ...
    draw() {
        ...
        this.heartBuffer.forEach(heart => {
            ...
            if (heart.y < -(heart.height)){
                ...
                heart.timeInit = this.configClock.timeLine // 重新定位时间节点
                heart.opacity = 0
            }

            // 当透明度到达最终透明度时
            let timeGap = this.configClock.timeLine - heart.timeInit // 时间为正数时才计算透明度
            if (timeGap > 0){
                heart.opacity = heart.opacity * ((this.configClock.timeLine - heart.timeInit)/100)
            ...
            // speed = 1/2 gt
            let movement = 1/2 * heart.gravity * (this.configClock.timeLine - heart.timeInit) / 300 * this.configClock.flowDirection
            ...
            this.drawHeart(
                ...
                heart.width / 2,
                heart.height / 2,
                `hsl(${heart.colorH} ${this.configClock.colorSaturate}% ${this.configClock.colorLight}% / ${heart.opacity}%)`
            )
            heart.opacity = heart.opacity + this.configClock.opacityGrowth
